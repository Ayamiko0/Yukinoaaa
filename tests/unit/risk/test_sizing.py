...
def test_fixed_fractional_position_sizing_accuracy() -> None:
    ...
    policy = RiskPolicy(
        max_risk_per_trade_percent=Decimal("0.02"), max_position_size_usd=Decimal("100000.00")
    )
...
def test_position_sizing_usd_cap_ceiling() -> None:
    ...
    policy = RiskPolicy(
        max_risk_per_trade_percent=Decimal("0.10"), max_position_size_usd=Decimal("1000.00")
    )
